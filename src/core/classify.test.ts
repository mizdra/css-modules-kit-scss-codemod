...
describe('convert findings', () => {
  ...
  test('classifies global ceil() and floor() as convertible by the expressions stage', () => {
    expect(findings('.a { width: ceil($a); }\n')).toEqual([
      { file: FILE, line: 1, column: 13, syntax: 'ceil', action: { kind: 'convert', stage: 'expressions' } },
    ]);
