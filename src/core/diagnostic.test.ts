...
test('shows no support planned when milestone is never', () => {
  ...
  expect(output).toBe('a.module.scss:1:1: Control flow is not supported. [@each, no support planned]\n');
});
...
test('shows planned: <milestone> when milestone is not never', () => {
  ...
  expect(output).toBe('a.module.scss:1:1: Color functions are not supported yet. [map value, planned: M2]\n');
});
...
test('omits absent optional fields from JSON formatting', () => {
  const output = formatDiagnosticsAsJson([{ file: 'a.module.scss', message: 'Failed to compile.' }]);

  expect(output).toBe(`${JSON.stringify([{ file: 'a.module.scss', message: 'Failed to compile.' }], null, 2)}\n`);
