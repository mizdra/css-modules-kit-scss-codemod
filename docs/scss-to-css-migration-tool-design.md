...
- [x] **Step 5: core/resolve + core/module-graph** — `@use`/`@forward`/Sass `@import` の specifier 抽出 → sass-loader 方式の `_` 展開 + oxc-resolver で解決 (相対 → node_modules → `--load-path` の順)。対象 glob 範囲外・`--exclude` 一致・`node_modules` 配下の参照先は無視 ([§5.1](#51-パーサー構成)) (853a55e, 93e7f77)
